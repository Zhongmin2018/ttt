<template>
    <div class="date_picker">
      <h1 class="title">不良品信息录入/查询<sup class="sup">BETA</sup></h1>
      <router-link :to="{ name: 'index'}" style="position: absolute; left:50px; top: 10px;"><img :src="home" alt="home" style="width: 38px;"></router-link><br/>
        <div class="date_input_box">
            ...
            <mu-auto-complete hintText="请输入不良类型" @input="handleInput" :dataSource="dataSource" @change="handlechange" v-model="reject_sketch"/>
            <mu-raised-button label="清除类型" class="demo-raised-button" @click="clearSketch"/>
            <mu-text-field hintText="请输入详情" v-model="reject_detail" :errorText="multiLineInputErrorText" @textOverflow="handleMultiLineOverflow" multiLine :rows="1" :rowsMax="20" :maxLength="300"/>
...
<script>
  ...
  export default {
        ...
        if(!vm.userName){
          ...
          axios.post('/api.php',{
            params: {
              ...
              'operate_event': 'checkRejectEnter',
            }
          }).then((res) => {
    ...
    beforeRouteLeave (to, from, next) {
      ...
      axios.post('/api.php',{
        params: {
          ...
          'operate_event': 'checkRejectLeave',
        }
      }).then((res) => {
        console.log(res);
        if(res.data.status == 'ok'){
    ...
    methods: {
      ...
      handleClick(e,item){
        this.reject_details = item.reject_detail;
        this.open();
      },
      clearSketch(){
        this.reject_sketch = "";
      ...
      handlechange (val) {
        this.reject_sketch = val;
      },
      handleInput (val) {
        const _self = this;
        console.log(_self.reject_sketch);
        axios.get('/api.php',{
          params:{
            'action': 'likeRejectSketch',
            'reject_sketch': _self.reject_sketch,
          }
        }).then((res) => {
          let data = []
          for(let i =0;i<res.data.length;i++){
            data.push(res.data[i].reject_sketch);
          }
      ...
      serch: function(){
        const _self = this;
        if(_self.ddns_name && _self.reject_sketch && _self.reject_detail){
          axios.put('/api.php',{
            params:{
              'action': 'updateDeviceInfo',
              'ddns_name': _self.ddns_name,
              'reject_sketch': _self.reject_sketch,
              'reject_detail': _self.reject_detail,
              'reject_input_name':_self.userName,
              'reject_input_time': 'setNow'
            }
          }).then((response) => {
        ...
        }else if(_self.ddns_name == '' && _self.reject_detail =='' && _self.reject_sketch){
          axios.get('/api.php',{
            params:{
              'action': 'getDeviceInfo',
              'reject_sketch': _self.reject_sketch
        ...
        }else if(_self.ddns_name && _self.reject_detail ==''){
          axios.get('/api.php',{
            params:{
              'action': 'getDeviceInfo',
              'ddns_name': _self.ddns_name
